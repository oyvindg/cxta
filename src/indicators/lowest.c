/**
 * @file lowest.c
 * @brief Rolling minimum (lowest) descriptor implementation.
 */
...
static double cxta_lowest_eval(const cxta_series_bar_view* view,
                               const double* args,
                               size_t nargs) {
    size_t idx;
    size_t start;
    ...
    window = (size_t)cxta_lowest_period_arg(args, nargs, 1);
    if (window > idx + 1u) window = idx + 1u;
    start = idx + 1u - window;
    value = view->bars[start].low;

    for (i = start + 1u; i <= idx; ++i) {
        if (view->bars[i].low < value) value = view->bars[i].low;
    }
    return value;
}

static double cxta_lowest_eval_scalar_src(const cxta_series_scalar_view* source,
                                          const double* args,
                                          size_t nargs) {
    size_t idx;
    size_t start;
...
const cxta_indicator_descriptor cxta_lowest_descriptor = {
    "lowest",
    1,
    1,
    1,
